...
function box(idnum) {
	...
	this.node.id=idnum;
	this.node.checked="0";
	this.node.innerHTML=this.node.id;//this.node.id; //temp
}
...
function displayId(box){
	...
	if (t.test(box.value)) box.name="1";	//if not bomb or 0 then =1
...
function checkClickedBox(id){
	...
	if (elem.value=="bomb")  alert("BOOOOM!!!");
}
